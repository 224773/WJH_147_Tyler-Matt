...
def logoPlace(original_image, logo_size, logo, st):
...
def logo_all_images(directory=None,size=0.25, st = False):
    
    if directory == None:
        directory = os.getcwd()
        
    new_directory = os.path.join(directory, 'logo_images')
    try:
    ...
    for n in range(len(image_list)):
        ...
        new_image = logoPlace(image_list[n], size, logo, st)
